"""
Individual MCP Tool Test: modify_order
Focus: Test modify_order MCP tool in isolation for debugging
MCP Tool: modify_order
Expected: Execute order modification through MCP interface
"""
...
@pytest.mark.paper
@pytest.mark.asyncio
class TestIndividualModifyOrder:
    """Test modify_order MCP tool in isolation"""
    
    async def test_modify_order_basic_functionality(self):
        """Test basic modify_order functionality through MCP interface"""
        
        print(f"\n{'='*60}")
        print(f"=== Testing MCP Tool: modify_order ===")
        print(f"{'='*60}")
        ...
        # MCP tool call with parameters - modify_order requires order_id and modification parameters
        tool_name = "modify_order"
        parameters = {
            "order_id": 12345,  # Example order ID (will likely fail in paper but test MCP interface)
            "quantity": 150,    # Modified quantity
            "price": 185.00     # Modified price (if limit order)
        }
        ...
        # modify_order will likely fail with "Order not found" but should handle gracefully
        ...
        if isinstance(parsed_result, dict):
            ...
            if "success" in parsed_result:
                ...
                if success:
                    ...
                    if "order_id" in parsed_result:
                        ...
                        print(f"[OK] Modified Order ID: {order_id}")
                        assert isinstance(order_id, int)
                    
                    if "modifications" in parsed_result:
                        modifications = parsed_result['modifications']
                        print(f"[OK] Modifications applied: {modifications}")
                else:
                    # Failed modification (expected for non-existent order)
                    if "error" in parsed_result:
                        error = parsed_result['error']
                        print(f"[OK] Expected error for non-existent order: {error}")
                        # This is actually successful behavior - order not found error is expected
            ...
            print(f"[PASS] ORDER MODIFICATION VALIDATION PASSED")
            
        else:
            print(f"Unexpected response format: {type(parsed_result)}")
            print(f"Response content: {parsed_result}")
            pytest.fail(f"Unexpected response format from MCP tool modify_order")
        
        print(f"\n[PASS] MCP Tool 'modify_order' test PASSED")
        print(f"{'='*60}")
        
    async def test_modify_order_error_handling(self):
        """Test modify_order error handling with invalid parameters"""
        
        print(f"\n{'='*60}")
        print(f"=== Testing Error Handling: modify_order ===")
        print(f"{'='*60}")
        
        # Test invalid parameters - missing order_id
        tool_name = "modify_order"
        invalid_parameters = {
            # Missing order_id - should trigger validation error
...
C:\Python313\python.exe -m pytest tests/paper/individual/test_individual_modify_order.py -v -s
...
C:\Python313\python.exe -m pytest tests/paper/individual/test_individual_modify_order.py::TestIndividualModifyOrder::test_modify_order_basic_functionality -v -s

# Full test class:
C:\Python313\python.exe -m pytest tests/paper/individual/test_individual_modify_order.py::TestIndividualModifyOrder -v -s
