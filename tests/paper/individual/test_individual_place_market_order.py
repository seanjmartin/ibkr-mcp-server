"""
Individual MCP Tool Test: place_market_order
Focus: Test place_market_order MCP tool in isolation for debugging
MCP Tool: place_market_order
Expected: Execute market order placement through MCP interface
"""
...
@pytest.mark.paper
@pytest.mark.asyncio
class TestIndividualPlaceMarketOrder:
    """Test place_market_order MCP tool in isolation"""
    
    async def test_place_market_order_basic_functionality(self):
        """Test basic place_market_order functionality through MCP interface"""
        
        print(f"\\n{'='*60}")
        print(f"=== Testing MCP Tool: place_market_order ===")
        print(f"{'='*60}")
        ...
        # For place_market_order, we expect either success or safety framework block
        if isinstance(parsed_result, dict):
            ...
            if parsed_result.get("success") is True:
                print(f"[OK] Order placement successful!")
                ...
                print(f"[PASSED] ORDER PLACEMENT VALIDATION PASSED")
            else:
                print(f"[ERROR] Unexpected response format: {parsed_result}")
                pytest.fail(f"Unexpected response from place_market_order")
            
        else:
            print(f"Unexpected response format: {type(parsed_result)}")
            print(f"Response content: {parsed_result}")
            pytest.fail(f"Unexpected response format from MCP tool place_market_order")
        
        print(f"\\n[SUCCESS] MCP Tool 'place_market_order' test PASSED")
        print(f"{'='*60}")
        
    async def test_place_market_order_error_handling(self):
        """Test place_market_order error handling with invalid parameters"""
        
        print(f"\\n{'='*60}")
        print(f"=== Testing Error Handling: place_market_order ===")
        print(f"{'='*60}")
...
C:\Python313\python.exe -m pytest tests/paper/individual/test_individual_place_market_order.py -v -s
...
C:\Python313\python.exe -m pytest tests/paper/individual/test_individual_place_market_order.py::TestIndividualPlaceMarketOrder::test_place_market_order_basic_functionality -v -s

# Full test class:
C:\Python313\python.exe -m pytest tests/paper/individual/test_individual_place_market_order.py::TestIndividualPlaceMarketOrder -v -s
