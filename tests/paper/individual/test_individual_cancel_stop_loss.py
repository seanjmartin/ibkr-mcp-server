"""
Individual MCP Tool Test: cancel_stop_loss
Focus: Test cancel_stop_loss MCP tool in isolation for debugging
MCP Tool: cancel_stop_loss
...
@pytest.mark.paper
@pytest.mark.asyncio
class TestIndividualCancelStopLoss:
    """Test cancel_stop_loss MCP tool in isolation"""
    
    async def test_cancel_stop_loss_non_existent_order(self):
        """Test cancel_stop_loss graceful handling of non-existent order through MCP interface"""
        
        print(f"\n{'='*60}")
        print(f"=== Testing MCP Tool: cancel_stop_loss (Non-Existent Order) ===")
        print(f"{'='*60}")
        ...
        if 'result' in locals():
            ...
            error_indicators = ["error", "not found", "invalid", "failed", "does not exist", "unknown", "cancel"]
        ...
        print(f"\n[PASSED] MCP Tool 'cancel_stop_loss' non-existent order test COMPLETED")
        print(f"{'='*60}")
        
    async def test_cancel_stop_loss_parameter_validation(self):
        """Test cancel_stop_loss parameter validation through MCP interface"""
        
        print(f"\n{'='*60}")
        print(f"=== Testing cancel_stop_loss Parameter Validation ===")
        print(f"{'='*60}")
        ...
        print(f"MCP Call: call_tool('{tool_name}', {parameters})")
        print(f"Testing parameter validation with invalid order_id=-1...")
...
C:\Python313\python.exe -m pytest tests/paper/individual/test_individual_cancel_stop_loss.py -v -s
...
C:\Python313\python.exe -m pytest tests/paper/individual/test_individual_cancel_stop_loss.py::TestIndividualCancelStopLoss::test_cancel_stop_loss_non_existent_order -v -s

# Full test class:
C:\Python313\python.exe -m pytest tests/paper/individual/test_individual_cancel_stop_loss.py::TestIndividualCancelStopLoss -v -s
