...
async def call_tool(name: str, arguments: dict[str, Any]) -> Sequence[TextContent]:
    ...
    try:
        ...
        # ============ FOREX TRADING TOOLS ============
            ...
            try:
                rates = await ibkr_client.get_forex_rates(currency_pairs)
                return [TextContent(
                    type="text",
                    text=json.dumps(rates, indent=2)
                )]
            except Exception as e:
                return [TextContent(
                    type="text",
        ...
        # ============ INTERNATIONAL TRADING TOOLS ============
            ...
            try:
                data = await ibkr_client.get_international_market_data(symbols, auto_detect)
                return [TextContent(
                    type="text",
        ...
        # ============ STOP LOSS MANAGEMENT TOOLS ============
            ...
            kwargs = {}
            for key in ["exchange", "currency", "action", "order_type", "limit_price", 
            ...
            try:
                result = await ibkr_client.place_stop_loss(
                    symbol=symbol, quantity=quantity, stop_price=stop_price, **kwargs
                )
                return [TextContent(
                    type="text",
                    text=json.dumps(result, indent=2)
            ...
            try:
                orders = await ibkr_client.get_stop_losses(account, symbol, status)
                return [TextContent(
                    type="text",
            ...
            try:
                result = await ibkr_client.modify_stop_loss(order_id, **modifications)
                return [TextContent(
                    type="text",
                    text=json.dumps(result, indent=2)
                )]
